...
"""Configuration for G1 robot with Inspire hands in the simple place task.
...
G1_INSPIRE_ASSETS_DIR = os.path.join(os.path.dirname(__file__), "../../../stack/config/g1_inspire/assets")
...
@configclass
class G1PlaceObservationsCfg:
    """Observation specifications for G1 place task (single cube)."""
...
@configclass
class PlaceEventCfg:
    """Configuration for place task events."""
...
@configclass
class PlaceTerminationsCfg:
    """Termination conditions for the place task."""

    # Success: cube in target zone with hands open
    success = DoneTerm(
        func=place_mdp.cube_in_target_zone,
        params={
...
@configclass
class G1InspireCubePlaceEnvCfg(StackEnvCfg):
    """Configuration for G1 Inspire simple place task: pick cube -> place in target zone.

    Inherits from StackEnvCfg to get working scene setup, then simplifies to single cube.
    """

    # Override observations
    observations: G1PlaceObservationsCfg = G1PlaceObservationsCfg()

    # Override terminations
    terminations: PlaceTerminationsCfg = PlaceTerminationsCfg()
    ...
    def __post_init__(self):
        ...
        # Set events for place task
        self.events = PlaceEventCfg()
        ...
        self.recorders = StreamingRecorderManagerCfg(
            dataset_export_dir_path="/workspace/isaaclab/datasets/place_g1_inspire",
