#!/usr/bin/env python3
"""
Convert StreamingRecorder JPEG/JSON dataset to LeRobot v3.0 format.

This script converts recorded demonstrations from the streaming format
(JPEG images + JSON state/action data) to LeRobot v3.0 format for unitree_lerobot training.
...
def create_directory_structure(output_path: Path, camera_keys: list[str], force: bool = False):
    """Create LeRobot v3.0 directory structure."""
    ...
    (output_path / "meta" / "episodes" / "chunk-000").mkdir(parents=True)  # v3.0 episodes parquet
    (output_path / "data" / "chunk-000").mkdir(parents=True)

    # Create video directories for each camera (v3.0 format: videos/{video_key}/chunk-000/)
...
def convert_episode(
    ...
    # Build frame data (matching working lerobot v3.0 format)
    frame_data = {
    ...
    # Encode videos (video paths are NOT stored in parquet - lerobot loads from episodes metadata)
    if not skip_videos:
        for cam_key, frames in camera_frames.items():
            if frames:
                video_key = CAMERA_MAPPING.get(cam_key, f"observation.images.{cam_key}")
                # v3.0 format: videos/{video_key}/chunk-000/file-{file_index}.mp4
                video_path = output_path / "videos" / video_key / "chunk-000" / f"file-{episode_idx:03d}.mp4"
...
def main():
    ...
    for ep_idx, episode_dir in enumerate(tqdm(episodes, desc="Converting episodes")):
        ...
        if num_frames > 0:
            # Write individual parquet file for this episode (v3.0 format: file-{idx}.parquet)
            df = pd.DataFrame(frame_data)
            parquet_path = output_path / "data" / "chunk-000" / f"file-{valid_episode_idx:03d}.parquet"
    ...
    info = {
        ...
        # LeRobot v3.0 path templates
        "data_path": "data/chunk-{chunk_index:03d}/file-{file_index:03d}.parquet",
        "video_path": "videos/{video_key}/chunk-{chunk_index:03d}/file-{file_index:03d}.mp4",
        "features": {
            # Index features (required by lerobot)
    ...
    tasks_df.index = pd.Index([args.task_description], name="task")
    tasks_df.to_parquet(output_path / "meta" / "tasks.parquet")

    # Build episodes metadata (v3.0 format: meta/episodes/chunk-000/file-000.parquet)
    frame_start = 0
    ep_counter = 0
    ...
    for episode_dir in episodes:
        ...
        ep_meta = {
            ...
            "data/chunk_index": 0,
            "data/file_index": ep_counter,
            "dataset_from_index": frame_start,
            "dataset_to_index": frame_start + num_frames,
            "tasks": [args.task_description],
            "length": num_frames,
            # Episode-level stats
            "stats/observation.state/min": ep_states.min(axis=0),
            "stats/observation.state/max": ep_states.max(axis=0),
            "stats/observation.state/mean": ep_states.mean(axis=0),
            "stats/observation.state/std": ep_states.std(axis=0),
            "stats/observation.state/count": np.array([num_frames]),
            "stats/action/min": ep_actions.min(axis=0),
            "stats/action/max": ep_actions.max(axis=0),
            "stats/action/mean": ep_actions.mean(axis=0),
            "stats/action/std": ep_actions.std(axis=0),
            "stats/action/count": np.array([num_frames]),
            "meta/episodes/chunk_index": 0,
            "meta/episodes/file_index": 0,
        }

        # Add video metadata for each camera
        episode_duration = num_frames / args.fps
        for cam_key in camera_keys:
            video_key = CAMERA_MAPPING.get(cam_key, f"observation.images.{cam_key}")
            ep_meta[f"videos/{video_key}/chunk_index"] = 0
            ep_meta[f"videos/{video_key}/file_index"] = ep_counter
            ep_meta[f"videos/{video_key}/from_timestamp"] = 0.0
            ep_meta[f"videos/{video_key}/to_timestamp"] = episode_duration
    ...
    # Write episodes parquet (v3.0 format)
    if episodes_metadata:
        episodes_df = pd.DataFrame(episodes_metadata)
        episodes_parquet_path = output_path / "meta" / "episodes" / "chunk-000" / "file-000.parquet"
        episodes_df.to_parquet(episodes_parquet_path, index=False)
